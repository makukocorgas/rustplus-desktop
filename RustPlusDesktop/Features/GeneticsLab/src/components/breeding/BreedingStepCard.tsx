...
export const BreedingStepCard: React.FC<BreedingStepCardProps> = ({
  ...
  return (
    <Paper
      ...
      sx={{
        p: 2.5,
        backgroundColor: step.isCompleted ? 'var(--gl-tint-green)' : isCurrent ? 'var(--gl-panel-header-bg)' : 'var(--gl-panel-bg)',
        border: '1.5px solid',
        borderColor: step.isCompleted ? 'var(--gl-success)' : isCurrent ? 'var(--gl-primary)' : 'var(--gl-border)',
        borderRadius: '6px',
        display: 'flex',
    ...
    >
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="caption" sx={{ color: 'var(--gl-text-secondary)', fontWeight: 700 }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', p: 1.5, backgroundColor: 'var(--gl-panel-bg)', border: '1px solid var(--gl-surface)', borderRadius: '4px' }}>
      ...
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
        ...
        {step.centerSaplingString && (
          <Box sx={{ p: 1.25, backgroundColor: 'var(--gl-input-bg)', border: '1px solid var(--gl-surface)', borderRadius: '4px' }}>
            <FormControlLabel
              control={
                <Checkbox
              ...
              label={
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                  <Typography variant="body2" sx={{ fontWeight: 700, color: 'var(--gl-text-primary)' }}>
            ...
            {step.surroundingSaplingsStrings.map((genes, pIdx) => {
              ...
              return (
                <Box key={pIdx} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Typography variant="caption" sx={{ color: 'var(--gl-text-muted)', minWidth: 16 }}>
                    #{pIdx + 1}
                  </Typography>
                  ...
                  {isSecond && (
                    <Typography variant="caption" sx={{ color: 'var(--gl-error)', fontWeight: 800, fontFamily: 'monospace' }}>
                      (2nd placement)
                    </Typography>
                  )}
                </Box>
        ...
        <Typography variant="caption" sx={{ color: 'var(--gl-text-muted)', fontStyle: 'italic', pl: 1 }}>
